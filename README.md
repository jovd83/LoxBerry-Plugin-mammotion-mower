...
| *Account or password mismatch* with **valid** credentials (works in the phone app) | Mammotion's server rejected the `App-Version` fingerprint, not the password. Open the plugin page → *Diagnostics* → set **App-Version fingerprint** to the latest [Mammotion-HA release tag](https://github.com/mikey0000/Mammotion-HA/releases) (e.g. `0.5.47`, without the leading `v`) and Save. Background: PyMammotion sends `App-Version: HA,2.<value>`; Mammotion's server only accepts strings that look like an official Mammotion-HA build. See [PyMammotion #137](https://github.com/mikey0000/PyMammotion/issues/137). |
